...
class Space extends Array {
  ...
  isVisible(metaWindow) {
    ...
    if (x + clone.width < min || x > min + workArea.width) {
      return false;
    } else {
      return true;
    }
  ...
  globalToScroll(gx, gy, useTarget = false) {
    ...
    let x =
      gx - this.monitor.x - (useTarget ? this.targetX : this.cloneContainer.x);
  ...
  createBackground() {
    ...
    this.signals.connect(
      ...
      (actor, event) => {
        ...
        let [ok, x, y] = this.actor.transform_stage_point(aX, aY);
...
class Spaces extends Map {
  ...
  init() {
    ...
    this.signals.connect(display, "grab-op-begin", (display, d, mw, type) =>
      grabBegin(mw, type)
    );
    this.signals.connect(display, "grab-op-end", (display, d, mw, type) =>
      grabEnd(mw, type)
    );
